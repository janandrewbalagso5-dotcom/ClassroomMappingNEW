<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Login.aspx.vb" Inherits="Login" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Login - Classroom Mapping System</title>
    <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&family=Poppins:wght@400;600;700&display=swap" rel="stylesheet" />
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }

        :root {
            --navy:        #0d1f4c;
            --navy-mid:    #162660;
            --navy-light:  #1e3278;
            --white:       #ffffff;
            --off-white:   #f5f7fc;
            --orange:      #f4df20;
            --orange-pale: #f3ea72;
            --pink:        #e8487a;
            --pink-pale:   #fce4ed;
            --text-muted:  #7a8bb5;
            --border:      #dce3f5;
            --shadow-card: 0 32px 80px rgba(13,31,76,0.22), 0 8px 24px rgba(13,31,76,0.12);
        }

        html, body {
            min-height: 100%;
            font-family: 'DM Sans', sans-serif;
            overflow-x: hidden;
        }

        /* ════════════════════════════════════════
           FULL-PAGE OMBRE — navy top → white bottom
        ════════════════════════════════════════ */
        .page {
            min-height: 100vh;
            width: 100%;
            background: linear-gradient(
                to bottom,
                var(--navy)      0%,
                var(--navy-mid)  18%,
                #2a3f80          35%,
                #6878b0          50%,
                #b8c2df          60%,
                #e2e7f4          63%,
                var(--navy-light)     100%, 
                var(--navy) 80%
            );
            display: flex;
            flex-direction: column;
            align-items: center;
            position: relative;
            overflow: hidden;
        }

        /* ── Decorative blobs ── */
        .blob {
            position: absolute;
            border-radius: 50%;
            pointer-events: none;
        }
        .blob-1 {
            width: 600px; height: 600px;
            background: radial-gradient(circle, rgba(244,124,32,0.12) 0%, transparent 65%);
            top: -100px; left: -160px;
        }
        .blob-2 {
            width: 480px; height: 480px;
            background: radial-gradient(circle, rgba(232,72,122,0.1) 0%, transparent 65%);
            top: 60px; right: -140px;
        }
        .blob-3 {
            width: 400px; height: 400px;
            background: radial-gradient(circle, rgba(30,50,120,0.06) 0%, transparent 65%);
            bottom: -80px; left: 50%;
            transform: translateX(-50%);
        }

        /* Decorative rings */
        .ring {
            position: absolute;
            border-radius: 50%;
            border: 1.5px solid rgba(255,255,255,0.08);
            pointer-events: none;
        }
        .ring-1 { width: 700px; height: 700px; top: -280px; left: -200px; }
        .ring-2 { width: 460px; height: 460px; top: -80px;  right: -180px; }

        /* ════════════════════════════════════════
           TOP NAV BAR — school branding
        ════════════════════════════════════════ */
        .topbar {
            width: 100%;
            max-width: 1100px;
            display: flex;
            flex-direction: column;
            align-items: center;
            gap: 16px;
            padding: 44px 40px 0;
            position: relative; z-index: 10;
            animation: fadeDown 0.5s ease both;
        }

        .school-logo {
            width: 96px; height: 96px;
            border-radius: 50%;
            object-fit: cover;
            border: 3.5px solid rgba(255,255,255,0.45);
            box-shadow: 0 8px 32px rgba(0,0,0,0.28);
            flex-shrink: 0;
        }
        .school-name {
            font-family: 'Playfair Display', serif;
            font-size: 17px;
            color: rgba(255,255,255,0.95);
            line-height: 1.45;
            text-align: center;
            max-width: 360px;
            font-weight: 600;
        }

        /* ════════════════════════════════════════
           HERO TEXT — above the card
        ════════════════════════════════════════ */
        .hero-block {
            text-align: center;
            margin-top: 36px;
            margin-bottom: 28px;
            position: relative; z-index: 10;
            animation: fadeDown 0.5s 0.08s ease both;
            opacity: 0;
        }

        .hero-eyebrow {
            display: inline-flex;
            align-items: center;
            gap: 7px;
            background: rgba(244,124,32,0.2);
            color: var(--orange);
            border: 1px solid rgba(244,124,32,0.4);
            border-radius: 20px;
            padding: 4px 14px;
            font-size: 10.5px;
            font-weight: 600;
            letter-spacing: 0.08em;
            text-transform: uppercase;
            margin-bottom: 16px;
        }
        .hero-eyebrow::before {
            content: '';
            width: 5px; height: 5px;
            border-radius: 50%;
            background: var(--orange);
            flex-shrink: 0;
        }

        .hero-title {
            font-family: 'Playfair Display', serif;
            font-size: clamp(28px, 4vw, 46px);
            color: #fff;
            line-height: 1.18;
            margin-bottom: 10px;
        }
        .hero-title em { font-style: normal; color: var(--orange); }

        .hero-sub {
            font-size: 14px;
            color: rgba(255,255,255,0.55);
            font-weight: 300;
        }

        /* ════════════════════════════════════════
           LOGIN CARD — centered, frosted
        ════════════════════════════════════════ */
        .login-card {
            background: rgba(255, 255, 255, 0.92);
            backdrop-filter: blur(18px);
            -webkit-backdrop-filter: blur(18px);
            border: 1px solid rgba(255,255,255,0.7);
            border-radius: 24px;
            box-shadow: var(--shadow-card);
            padding: 46px 52px 42px;
            width: 100%;
            max-width: 460px;
            position: relative;
            z-index: 10;
            animation: fadeUp 0.5s 0.18s ease both;
            opacity: 0;
        }

        /* Gradient accent bar */
        .login-card::before {
            content: '';
            position: absolute;
            top: 0; left: 0; right: 0;
            height: 4px;
            border-radius: 24px 24px 0 0;
        }

        /* Card header */
        .card-header { margin-bottom: 26px; }
        .card-header h1 {
            font-family: 'Playfair Display', serif;
            font-size: 26px;
            color: var(--navy);
            font-weight: 700;
            margin-bottom: 5px;
        }
        .card-header p {
            font-size: 13px;
            color: var(--text-muted);
            font-weight: 300;
        }

        /* Form groups */
        .form-group { margin-bottom: 16px; }
        .form-group label {
            display: block;
            font-size: 10.5px;
            font-weight: 600;
            color: var(--navy);
            letter-spacing: 0.08em;
            text-transform: uppercase;
            margin-bottom: 7px;
        }

         /* Role dropdown styling */
        .role-dropdown {
            appearance: none;
            -webkit-appearance: none;
            cursor: pointer;
        }

        .dropdown-arrow {
            position: absolute;
            right: 13px;
            top: 50%;
            transform: translateY(-50%);
            pointer-events: none;
            color: var(--text-muted);
            display: flex;
            align-items: center;
            transition: color 0.2s;
        }

        .input-wrap:focus-within .dropdown-arrow {
            color: var(--navy-light);
}

        /* Input wrap */
        .input-wrap { position: relative; }
        .input-icon {
            position: absolute;
            left: 13px; top: 50%;
            transform: translateY(-50%);
            pointer-events: none;
            opacity: 0.3;
            transition: opacity 0.2s;
        }
        .input-wrap:focus-within .input-icon { opacity: 1; }
        .input-wrap:focus-within .input-icon svg { stroke: var(--navy-light); }

        /* Inputs */
        .login-card input[type=text],
        .login-card input[type=password],
        .form-control {
            width: 100%;
            padding: 11px 13px 11px 40px;
            border: 1.5px solid var(--border);
            border-radius: 10px;
            font-size: 13.5px;
            font-family: 'DM Sans', sans-serif;
            background: var(--off-white);
            color: var(--navy);
            transition: border-color 0.2s, box-shadow 0.2s, background 0.2s;
            appearance: none;
            outline: none;
        }
        .login-card input[type=text]:focus,
        .login-card input[type=password]:focus,
        .form-control:focus {
            border-color: var(--navy-light);
            background: var(--white);
            box-shadow: 0 0 0 3.5px rgba(30,50,120,0.09);
        }

        /* Password toggle */
        .toggle-pw {
            position: absolute;
            right: 13px; top: 50%;
            transform: translateY(-50%);
            background: none; border: none;
            cursor: pointer; padding: 0;
            color: var(--text-muted);
            display: flex; align-items: center;
            transition: color 0.2s;
        }
        .toggle-pw:hover { color: var(--navy); }

        /* Sign In button */
        .btn-login, input[type="submit"].btn-login {
            width: 100%;
            padding: 13px;
            background: linear-gradient(135deg, var(--navy-light), var(--navy));
            color: #fff;
            border: none;
            border-radius: 11px;
            font-size: 14px;
            font-family: 'DM Sans', sans-serif;
            font-weight: 600;
            letter-spacing: 0.04em;
            cursor: pointer;
            margin-top: 4px;
            position: relative;
            overflow: hidden;
            transition: box-shadow 0.22s, transform 0.14s;
        }
        .btn-login::before {
            content: '';
            position: absolute; inset: 0;
            background: linear-gradient(135deg, rgba(244,124,32,0.16), rgba(232,72,122,0.12));
            opacity: 0;
            transition: opacity 0.22s;
        }
        .btn-login:hover { box-shadow: 0 10px 32px rgba(13,31,76,0.28); transform: translateY(-2px); }
        .btn-login:hover::before { opacity: 1; }
        .btn-login:active { transform: translateY(0); }

        /* Error */
        .msg-error {
            display: block;
            background: #fff0f3;
            color: #c0274e;
            border: 1px solid #f7b8cb;
            border-radius: 9px;
            padding: 10px 14px;
            font-size: 13px;
            text-align: center;
            margin-top: 14px;
        }

        /* Divider */
        .divider {
            display: flex; align-items: center; gap: 12px;
            margin: 20px 0 16px;
            font-size: 12px;
            color: var(--text-muted);
        }
        .divider::before, .divider::after {
            content: ''; flex: 1;
            height: 1px; background: var(--border);
        }

        /* Sign-up row */
        .signup-row {
            text-align: center;
            font-size: 13px;
            color: var(--text-muted);
        }
        .signup-row a {
            color: var(--navy-light);
            font-weight: 600;
            text-decoration: none;
            border-bottom: 1.5px solid transparent;
            transition: border-color 0.18s, color 0.18s;
        }
        .signup-row a:hover { color: var(--pink); border-color: var(--pink); }


        /* Footer */
        .page-footer {
            font-size: 11.5px;
            color: rgba(13,31,76,1.5);
            text-align: center;
            padding: 20px 20px 28px;
            position: relative; z-index: 10;
        }

        /* Animations */
        @keyframes fadeDown {
            from { opacity: 0; transform: translateY(-16px); }
            to   { opacity: 1; transform: translateY(0); }
        }
        @keyframes fadeUp {
            from { opacity: 0; transform: translateY(18px); }
            to   { opacity: 1; transform: translateY(0); }
        }

        /* Responsive */
        @media (max-width: 560px) {
            .login-card { padding: 32px 24px 28px; }
            .hero-title { font-size: 28px; }
            .topbar { padding: 24px 20px 0; }
        }
    </style>
</head>
<body>
    <form id="form1" runat="server">
        <div class="page">

            <!-- Decorative blobs & rings -->
            <div class="blob blob-1"></div>
            <div class="blob blob-2"></div>
            <div class="blob blob-3"></div>
            <div class="ring ring-1"></div>
            <div class="ring ring-2"></div>

             <!-- ── Top bar — school branding ── -->
             <div class="topbar">
                 <img src="styles/logo.png" alt="OLPC Logo" class="school-logo" />
                 <span class="school-name">Our Lady of the Pillar College—San Manuel Inc.</span>
             </div>

             <!-- ── Hero text ── -->
             <div class="hero-block">
                 <h1 class="hero-title">Classroom <em>Mapping</em> System</h1>
                 <p class="hero-sub">Academic Information System</p>
             </div>

             <!-- ── Login card ── -->
             <div class="login-card">

                 <div class="card-header">
                     <h1>Welcome back</h1>
                     <p>Sign in to access your dashboard</p>
                 </div>

                         <!-- Role dropdown -->
                    <div class="form-group">
                        <label>Sign in as</label>
                        <div class="input-wrap">
                            <span class="input-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24"
                                     fill="none" stroke="#0d1f4c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>
                                    <circle cx="9" cy="7" r="4"/>
                                    <path d="M23 21v-2a4 4 0 0 0-3-3.87"/>
                                    <path d="M16 3.13a4 4 0 0 1 0 7.75"/>
                                </svg>
                            </span>
                            <asp:DropDownList ID="ddlRole" runat="server" CssClass="form-control role-dropdown">
                                <asp:ListItem Value="">-- Select User Type --</asp:ListItem>
                                <asp:ListItem Value="Admin">Admin</asp:ListItem>
                                <asp:ListItem Value="Dean">Dean</asp:ListItem>
                                <asp:ListItem Value="Instructor">Instructor</asp:ListItem>
                                <asp:ListItem Value="Student">Student</asp:ListItem>
                            </asp:DropDownList>
                            <span class="dropdown-arrow">
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"
                                     fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <polyline points="6 9 12 15 18 9"/>
                                </svg>
                            </span>
                        </div>
                    </div>

                  <!-- Username -->
                <div class="form-group">
                    <label>Username</label>
                    <div class="input-wrap">
                        <span class="input-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24"
                                 fill="none" stroke="#0d1f4c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
                                <circle cx="12" cy="7" r="4"/>
                            </svg>
                        </span>
                        <asp:TextBox ID="txtUsername" runat="server" CssClass="form-control"
                                    placeholder="Enter your username" autocomplete="username" />
                    </div>
                </div>

                <div class="form-group">
                    <label>Password</label>
                    <div class="input-wrap" style="position:relative;">
                          <span class="input-icon">
                              <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24"
                                   fill="none" stroke="#0d1f4c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                  <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
                                  <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                              </svg>
                          </span>
                        <asp:TextBox ID="txtPassword" runat="server" TextMode="Password" placeholder="Enter password" style="padding-right: 40px;" />
                        <button type="button" id="togglePassword" onclick="togglePwd()" 
                            style="position:absolute; right:12px; background:none; border:none; cursor:pointer; color:#aaa; font-size:16px; padding:0; display:flex; align-items:center; top:12px;">
                             <svg id="eyeIcon" xmlns="http://www.w3.org/2000/svg" width="15" height="15"
                                  viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                  stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                 <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                 <circle cx="12" cy="12" r="3"/>
                             </svg>
                            <svg id="eyeOffIcon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:none;">
                                <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/>
                                <path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/>
                                <line x1="1" y1="1" x2="23" y2="23"/>
                            </svg>
                        </button>
                    </div>
                </div>


                <script>
                    function togglePwd() {
                        var pwdBox = document.getElementById('<%= txtPassword.ClientID %>');
                        var eyeOn  = document.getElementById('eyeIcon');
                        var eyeOff = document.getElementById('eyeOffIcon');
                        var btn    = document.getElementById('togglePassword');

                        if (pwdBox.type === 'password') {
                            pwdBox.type = 'text';
                            eyeOn.style.display  = 'none';
                            eyeOff.style.display = 'block';
                            btn.style.color = '#4a6fa5';
                        } else {
                            pwdBox.type = 'password';
                            eyeOn.style.display  = 'block';
                            eyeOff.style.display = 'none';
                            btn.style.color = '#aaa';
                        }
                    }

                    function selectRole(el) {
                        document.querySelectorAll('.role-opt').forEach(function (o) {
                            o.classList.remove('active', 'orange-accent', 'pink-accent');
                        });
                        el.classList.add('active');
                        var v = el.getAttribute('data-val');
                        if (v === 'Instructor' || v === 'Student') el.classList.add('orange-accent');
                        else if (v === 'Dean') el.classList.add('pink-accent');
                        var ddl = document.getElementById('<%= ddlRole.ClientID %>');
                        if (ddl) {
                            for (var i = 0; i < ddl.options.length; i++) {
                                if (ddl.options[i].value === v) { ddl.selectedIndex = i; break; }
                            }
                        }
                    }

                    window.onload = function () {
                        var ddl = document.getElementById('<%= ddlRole.ClientID %>');
                        if (ddl && ddl.value) {
                            var pill = document.querySelector('.role-opt[data-val="' + ddl.value + '"]');
                            if (pill) selectRole(pill);
                        }
                    };

                    function togglePassword() {
                        var pw = document.getElementById('<%= txtPassword.ClientID %>');
                        var icon = document.getElementById('eyeIcon');
                        if (!pw) return;
                        if (pw.type === 'password') {
                            pw.type = 'text';
                            icon.innerHTML =
                                '<path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/>' +
                                '<path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/>' +
                                '<line x1="1" y1="1" x2="23" y2="23"/>';
                        } else {
                            pw.type = 'password';
                            icon.innerHTML =
                                '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>';
                        }
                    }
                 </script>

                 <!-- Sign In -->
                 <asp:Button ID="btnLogin" runat="server" Text="Sign In →" CssClass="btn-login" />
                 <asp:Label ID="lblError" runat="server" CssClass="msg-error" Visible="false"></asp:Label>

                 <div class="divider">or</div>
                 <div class="signup-row">
                     Don't have an account? <a href="SignUp.aspx">Create one</a>
                 </div>

               </div><%-- /login-card --%>

              <div class="page-footer">
                 Classroom Mapping System &copy; 2026 &nbsp;·&nbsp; Our Lady of the Pillar College—San Manuel Inc.
             </div>

        </div>
    </form>
</body>
</html>
